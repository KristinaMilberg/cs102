...
class FriendsTestCase(unittest.TestCase):
    ...
    def test_get_mutual(self):
        ...
        responses.add(
            responses.GET,
            re.compile(
                f"https://api.vk.com/method/friends.getMutual\?.*source_uid={source_uid}.*target_uids={target_uid}.*"
            ),
            match_querystring=True,
            json={"response": common_friends},
            status=200,
        )
        mutual_friends = get_mutual(source_uid=source_uid, target_uids=[target_uid])
    ...
    def test_get_mutual_more_than100(self):
        responses.add(
            responses.GET,
            re.compile("https://api.vk.com/method/friends.getMutual\?.*source_uid=123.*offset=0.*"),
            match_querystring=True,
        ...
        responses.add(
            responses.GET,
            re.compile("https://api.vk.com/method/friends.getMutual\?.*source_uid=123.*offset=100.*"),
            match_querystring=True,
        ...
        responses.add(
            responses.GET,
            re.compile("https://api.vk.com/method/friends.getMutual\?.*source_uid=123.*offset=200.*"),
            match_querystring=True,
            json={"response": [{"id": 3, "common_friends": [1, 2], "common_count": 2}]},
            status=200,
        )

        mutual_friends = get_mutual(source_uid=123, target_uids=list(range(300)))
        self.assertEqual(
            [
    ...
    def test_get_mutual_too_many_requests_handled_properly(self):
        ...
        n_reqs = 4
        start = time.time()
        mutual_friends = get_mutual(source_uid=123, target_uids=list(range(n_reqs * 100)))
        end = time.time()
