...
def find_possible_values(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.Set[str]:
    ...
    True
    """
    used_values = set(get_block(grid, pos) + get_col(grid, pos) + get_row(grid, pos))
    return set(map(str, range(1, 10))) - used_values


def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:  # type: ignore
...
def check_solution(solution: tp.List[tp.List[str]]) -> bool:
    ...
    for i in range(len(solution)):
        for j in range(len(solution[0])):
            block, col, row = (
                set(get_block(solution, (i, j))),
                set(get_col(solution, (i, j))),
                set(get_row(solution, (i, j))),
            )
            if row != nums or col != nums or block != nums:
                return False
    return True


def generate_sudoku(N: int) -> tp.List[tp.List[str]]:  # type: ignore
