...
def get_exits(grid: List[List[Union[str, int]]]) -> List[Tuple[int, int]]:
    ...
    for i in range(0, len(grid)):
        ...
        possible_exits.append([len(grid) - 1, i])
        possible_exits.append([i, len(grid) - 1])
    for j in range(len(possible_exits)):
        x, y = possible_exits[j][0], possible_exits[j][1]
...
def solve_maze(
    ...
    lenx = len(grid)
    leny = len(grid[0])
    coordinates = get_exits(grid)
    if len(coordinates) == 1:
        return grid, coordinates[0]
    if not encircled_exit(grid, coordinates[0]) and not encircled_exit(grid, coordinates[1]):
        for x in range(lenx):
            for y in range(leny):
                if grid[x][y] == " ":
                    grid[x][y] = 0
        grid[coordinates[0][0]][coordinates[0][1]] = 1
        grid[coordinates[1][0]][coordinates[1][1]] = 0
        m = 1
        while grid[coordinates[1][0]][coordinates[1][1]] == 0:
            grid = make_step(grid, m)
            m += 1
        route = shortest_path(grid, coordinates[1])
        for x in range(lenx):
            for y in range(leny):
